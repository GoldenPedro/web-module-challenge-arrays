...
function is31Flavors(arr){
    ...
    if (arr.length = 31) {
        ...
        var isLength31 = false;
    }

    console.log(isLength31);
}
...
function removeFlavorByName(arr, flavorRemoved){
    for (i=0; i<arr.length; i++) {
        if (arr[i] === flavorRemoved) {
            arr.splice(i, 1)
            break;
        }
    }  
...
function getRandomFlavors(arr1, arr2, arr3, arr4){
    ...
    for (i = 0; i < 31; i++){
        finishedArr.push(newArr[Math.floor(Math.random() * newArr.length)]);
    }
